...
class NotificationService:
    ...
    @staticmethod
    def _format_testnet_orders(testnet_orders: dict) -> list[str]:
        """Format testnet order details into message lines."""
        lines = ["--- Testnet Orders ---"]
        entry = testnet_orders.get('entry')
        if entry:
            fill_price = entry.get('avgPrice', entry.get('price', 'N/A'))
            lines.append(f"Entry: orderId={entry.get('orderId')} status={entry.get('status')} avgPrice={fill_price}")
        sl = testnet_orders.get('stop_loss')
        if sl:
            lines.append(f"Stop Loss: orderId={sl.get('orderId')} stopPrice={sl.get('stopPrice', 'N/A')} status={sl.get('status')}")
        tp = testnet_orders.get('take_profit')
        if tp:
            lines.append(f"Take Profit: orderId={tp.get('orderId')} stopPrice={tp.get('stopPrice', 'N/A')} status={tp.get('status')}")
        close = testnet_orders.get('close')
        if close:
            fill_price = close.get('avgPrice', close.get('price', 'N/A'))
            lines.append(f"Close: orderId={close.get('orderId')} status={close.get('status')} avgPrice={fill_price}")
        return lines

    def send_trade_notification(self, symbol, action, price, units, reason=None, pnl=None,
                                stop_loss_price=None, is_volume_on: bool = False,
                                testnet_orders: dict = None):
        ...
        if testnet_orders:
            message_parts.extend(self._format_testnet_orders(testnet_orders))
